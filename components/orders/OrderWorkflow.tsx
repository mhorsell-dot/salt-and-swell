...
export default function OrderWorkflow({
  ...
  return (
    <div className="rounded-[2rem] bg-white p-8 shadow-sm">
      ...
      <h2 className="mt-4 text-3xl font-semibold tracking-tight">{currentStatus}</h2>
      ...
      <div className="mt-8 space-y-4">
        {workflow.map((step) => {
          ...
          const completed =
            workflow.findIndex((item) => item.status === currentStatus) >=
            workflow.findIndex((item) => item.status === step.status);
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`flex h-11 w-11 items-center justify-center rounded-full ${
                    completed ? "bg-[#182321] text-white" : "bg-black/5 text-black/40"
                  }`}
                >
              ...
              <button
                ...
                className={`rounded-full px-5 py-3 text-xs font-semibold uppercase tracking-wider ${
                  completed ? "bg-black/10 text-black/40" : "bg-black text-white"
                }`}
              >
